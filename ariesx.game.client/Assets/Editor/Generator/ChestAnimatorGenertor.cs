...
namespace Poukoute {
    ...
    public class ChestAnimatorGenerator : MonoBehaviour {
        ...
        [MenuItem("Poukoute/Generator/Generate Animator/Chest")]
        ...
        private static void GenerateRolePrefab(string heroName, string direction) {
            string fullPath = string.Format("{0}/Chest_{1}.controller", heroAnimatorPath, heroName);
            string absPath = Application.dataPath + "/" + fullPath;
            AnimatorController ac = AnimatorController.CreateAnimatorControllerAtPath(fullPath);
            ...
            string assetPath = string.Format("{0}idle_{1}.prefab",
            ...
            GameHelper.SetLayer(moveHero, LayerMask.NameToLayer("Default"));
            Debug.Log(ac);
            ...
            moveHero.name = "Hero";
            newPrefab.layer = 24;
            moveHero.transform.GetChild(1).gameObject.layer = 24;
