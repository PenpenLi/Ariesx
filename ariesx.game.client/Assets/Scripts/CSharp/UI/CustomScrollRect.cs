...
namespace UnityEngine.UI {
    [AddComponentMenu("UI/Custom Scroll Rect", 38)]
    [SelectionBase]
    [ExecuteInEditMode]
    ...
    public class CustomScrollRect: UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup {
        ...
        public Vector2 m_FrameOffset = Vector2.zero;
        ...
        private UnityEvent m_onContentPosChanged = new UnityEvent();
        public UnityEvent onContentPosChanged {
            get {
                return this.m_onContentPosChanged;
            }
        }
        ...
        public virtual void OnBeginDrag(PointerEventData eventData) {
            ...
            m_FrameOffset = Vector2.zero;

            this.onBeginDrag.Invoke();
        }

        public virtual void OnEndDrag(PointerEventData eventData) {
            AudioManager.Play("act_scroll_roll", Poukoute.AudioType.Action, AudioVolumn.High);
            if (eventData.button != PointerEventData.InputButton.Left)
                return;
        ...
        public virtual void SetContentAnchoredPosition(Vector2 position) {
            ...
            this.onContentPosChanged.Invoke();
        }
        ...
        protected virtual void LateUpdate() {
            ...
            if (!m_Dragging && (offset.sqrMagnitude > 0.04f || m_Velocity != Vector2.zero)) {
                Vector2 position = m_Content.anchoredPosition;
                for (int axis = 0; axis < 2; axis++) {
                    // Apply spring physics if movement is elastic and content has an offset from the view.
                    if (m_MovementType == MovementType.Elastic && offset[axis].Abs() > 0.2f) {
            ...
            if (m_Dragging && m_Inertia) {
                Vector3 newVelocity = (m_Content.anchoredPosition - m_PrevPosition - m_FrameOffset) / deltaTime;
            ...
            if (m_HorizontalScrollbar) {
                if (m_ContentBounds.size.x > 0)
                    m_HorizontalScrollbar.size =
                        Mathf.Clamp01((m_ViewBounds.size.x - offset.x.Abs()) / m_ContentBounds.size.x);
                else
            ...
            if (m_VerticalScrollbar) {
                if (m_ContentBounds.size.y > 0)
                    m_VerticalScrollbar.size =
                        Mathf.Clamp01((m_ViewBounds.size.y - offset.y.Abs()) / m_ContentBounds.size.y);
                else
        ...
        private void SetNormalizedPosition(float value, int axis) {
            ...
            if ((localPosition[axis] - newLocalPosition).Abs() > 0.01f) {
        ...
        private static float RubberDelta(float overStretching, float viewSize) {
            return (1 - (1 / ((overStretching.Abs() * 0.55f / viewSize) + 1))) * viewSize * Mathf.Sign(overStretching);
        }
        ...
        void UpdateScrollbarVisibility() {
            if (m_VerticalScrollbar && m_VerticalScrollbarVisibility != ScrollbarVisibility.Permanent && m_VerticalScrollbar.gameObject.activeSelf != vScrollingNeeded)
                m_VerticalScrollbar.gameObject.SetActiveSafe(vScrollingNeeded);
            ...
            if (m_HorizontalScrollbar && m_HorizontalScrollbarVisibility != ScrollbarVisibility.Permanent && m_HorizontalScrollbar.gameObject.activeSelf != hScrollingNeeded)
                m_HorizontalScrollbar.gameObject.SetActiveSafe(hScrollingNeeded);
        }
