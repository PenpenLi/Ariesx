...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter {
    ...
    abstract class InternalStateGeneric<TParams, TItemViewsHolder>
        ...
        internal double ContentPanelVirtualInsetFromViewportStart {
            get {
                return contentPanelSkippedInsetDueToVirtualization +
                    _SourceParams.content.GetInsetFromParentEdge(_SourceParams.viewport, startEdge);
            }
        }

        internal double ContentPanelVirtualInsetFromViewportEnd {
            get {
                return -contentPanelVirtualSize + viewportSize -
                    ContentPanelVirtualInsetFromViewportStart;
            }
        }
        ...
        internal float RealScrollableArea {
            get {
                return contentPanelSize - viewportSize;
            }
        }
        ...
        public virtual void Updateparams(TParams sourceParams) {
        ...
        protected InternalStateGeneric(TParams sourceParams, ItemsDescriptor itemsDescriptor) {
            ...
            if (layoutElement) {
                Debug.Log("LayoutElement on GameObject " + layoutElement.name + " has beed DESTROYED in order to use ScrollRectItemsAdapter8");
        ...
        internal void CacheScrollViewInfo() {
            ...
            if (_SourceParams.scrollRect.horizontal) {
                ...
                _ItemsDesc.itemsConstantTransversalSize =
                    _SourceParams.content.rect.height -
                    (transversalPaddingContentStart + _SourceParams.contentPadding.bottom);
            } else {
                ...
                _ItemsDesc.itemsConstantTransversalSize =
                    _SourceParams.content.rect.width -
                    (transversalPaddingContentStart + _SourceParams.contentPadding.right);
            }
        ...
        internal void UpdateLastProcessedCTVirtualInsetFromParentStart() {
            lastProcessedCTVirtualInsetFromParentStart = ContentPanelVirtualInsetFromViewportStart;
        }
        ...
        internal float GetItemInferredRealInsetFromParentStart(int itemIndexInView) { return ConvertItemInsetFromParentStart_FromVirtualToReal(GetItemVirtualInsetFromParentStartUsingItemIndexInView(itemIndexInView)); }
        internal float GetItemInferredRealInsetFromParentEnd(int itemIndexInView) { return contentPanelSize - GetItemInferredRealInsetFromParentStart(itemIndexInView) - _ItemsDesc[itemIndexInView]; }

        internal float ConvertItemInsetFromParentStart_FromVirtualToReal(double virtualInsetFromParrentStart) { return (float)(virtualInsetFromParrentStart + contentPanelSkippedInsetDueToVirtualization); }

        void OnCumulatedSizesOfAllItemsChanged(
            bool contentPanelEndEdgeStationary, bool rebuild = true) {
            _ItemsDesc.cumulatedSizesOfAllItemsPlusSpacing =
                _ItemsDesc.CumulatedSizeOfAllItems + Math.Max(0d, _ItemsDesc.itemsCount - 1) * spacing;
            OnCumulatedSizesOfAllItemsPlusSpacingChanged(
                contentPanelEndEdgeStationary, rebuild);
        }
