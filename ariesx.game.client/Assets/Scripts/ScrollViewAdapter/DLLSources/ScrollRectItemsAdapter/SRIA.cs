...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter {
    ...
    public abstract partial class SRIA<ViewModelType, TItemViewsHolder> : BaseView, ISRIA, IBeginDragHandler, IEndDragHandler
    where ViewModelType : BaseViewModel
    where TItemViewsHolder : BaseItemViewsHolder {
        /// <summary>
        /// Temp add ListView propeties into SRIA
        /// </summary>
        ...
        public event Action DataRequestAction;
        #endregion
        ...
        /// <summary>Becomes true after <see cref="SRIA{TParams, TItemViewsHolder}.SRIAInit"/> and false in <see cref="SRIA{TParams, TItemViewsHolder}.Dispose"/></summary>
        ...
        public void UpdateItemsDesc(int itemIndex, float requestedSize) {
            _ItemsDesc.BeginChangingItemsSizes(itemIndex);
            _ItemsDesc[itemIndex] = requestedSize;
            _ItemsDesc.EndChangingItemsSizes();
        }

        public void UpdateContentGravity(BaseParams.ContentGravity contentGravity = BaseParams.ContentGravity.START) {
            this.ItemsRefreshed = null;
            if (_Params.contentGravity != contentGravity) {
                _Params.contentGravity = contentGravity;
                _InternalState.Updateparams(_Params);
                this.Refresh();
        ...
        public void SetNormalizedPosition(float normalizedPosition) {
            float abstractNormPos = _Params.scrollRect.horizontal ?
        ...
        protected void SRIAInit(CustomScrollRect customScrollRect, HorizontalOrVerticalLayoutGroup layoutGroup,
            BaseParams.ContentGravity contentGravity = BaseParams.ContentGravity.START, float defaultItemSize = 90f) {
            this.SRIAInit(customScrollRect, layoutGroup.padding, layoutGroup.spacing, contentGravity, defaultItemSize);
        }
        ...
        protected void SRIAInit(CustomScrollRect customScrollRect, RectOffset contentPadding, float contentSpacing = 0.0f,
            BaseParams.ContentGravity contentGravity = BaseParams.ContentGravity.START, float defaultItemSize = 90f) {
            Canvas.ForceUpdateCanvases();
            _Params = new BaseParams(customScrollRect) {
            ...
            if (_Params.scrollRect.horizontalScrollbar != null ||
                _Params.scrollRect.verticalScrollbar != null)
                throw new UnityException(
                    "SRIA only works with a ScrollbarFixer8 component " +
                    "added to the Scrollbar and the ScrollRect shouldn't have any scrollbar " +
                    "set up in the inspector (it hooks up automatically)"
                );
        ...
        public virtual void Refresh() {
            ChangeItemsCount(ItemCountChangeMode.RESET, _ItemsDesc.itemsCount);
        }
        ...
        public virtual void ResetItems(int itemsCount, bool contentPanelEndEdgeStationary = false,
            bool keepVelocity = false, bool needRefresh = true) {
            //if (itemsCount > 0) {
        ...
        public virtual void InsertItems(int index,
                                        int itemsCount,
                                        bool contentPanelEndEdgeStationary = false,
                                        bool keepVelocity = true) {
            if (itemsCount > 0) {
                ChangeItemsCount(ItemCountChangeMode.INSERT,
                    itemsCount,
                    index,
        ...
        /// <summary>It preserves previously cached sizes</summary>
        public virtual void RemoveItems(int index,
                                        int itemsCount,
                                        bool contentPanelEndEdgeStationary = false,
                                        bool keepVelocity = false) {
            if (itemsCount > 0) {
                ChangeItemsCount(ItemCountChangeMode.REMOVE,
                itemsCount,
                index,
                contentPanelEndEdgeStationary,
                keepVelocity);
        ...
        public virtual void ScrollTo(int itemIndex, float normalizedOffsetFromViewportStart = 0.25f, float normalizedPositionOfItemPivotToUse = 0f) {
        ...
        public virtual bool SmoothScrollTo(int itemIndex, float duration, float normalizedOffsetFromViewportStart = 0f, float normalizedPositionOfItemPivotToUse = 0f, Func<float, bool> onProgress = null, bool overrideCurrentScrollingAnimation = false) {
            ...
            _SmoothScrollCoroutine = StartCoroutine(SmoothScrollProgressCoroutine(
                    itemIndex, duration,
                    normalizedOffsetFromViewportStart,
                    normalizedPositionOfItemPivotToUse,
                    onProgress));
        ...
        public float RequestChangeItemSizeAndUpdateLayout(
            TItemViewsHolder withVH, float requestedSize,
            bool itemEndEdgeStationary = false, bool computeVisibility = true) {
            return RequestChangeItemSizeAndUpdateLayout(withVH.ItemIndex,
                requestedSize, itemEndEdgeStationary, computeVisibility);
        }
        ...
        public float RequestChangeItemSizeAndUpdateLayout(int itemIndex, float requestedSize,
            bool itemEndEdgeStationary = false, bool computeVisibility = true) {
            CancelAnimationsIfAny();
            ...
            float resolvedSize = _InternalState.ChangeItemSizeAndUpdateContentSizeAccordingly(
        ...
        public double GetVirtualAbstractNormalizedScrollPosition() {
            return _InternalState.GetVirtualAbstractNormalizedScrollPosition();
        }
        ...
        public void SetVirtualAbstractNormalizedScrollPosition(double pos, bool computeVisibilityNow) {
            ...
            if (computeVisibilityNow && executed) {
                ...
                ScrollPositionChanged(normalizedPos);
            }

            if (normalizedPos < 0.1f && DataRequestAction != null) {
                DataRequestAction();
                DataRequestAction = null;
            }
        ...
        protected virtual bool IsRecyclable(TItemViewsHolder potentiallyRecyclable, int indexOfItemThatWillBecomeVisible, float heightOfItemThatWillBecomeVisible) { return true; }
        ...
        protected virtual bool ShouldDestroyRecyclableItem(TItemViewsHolder inRecycleBin, bool isInExcess) { return isInExcess; }
            ...
            if (_RecyclableItems != null) {
                foreach (var recyclable in _RecyclableItems) {
                    if (recyclable != null && recyclable.root != null)
                        try { PoolManager.RemoveObject(recyclable.root.gameObject); } catch (Exception e) { UnityEngine.Debug.LogException(e); }
                }
            ...
            if (_VisibleItems != null) {
                foreach (var item in _VisibleItems) {
                    if (item != null && item.root != null)
                        try { PoolManager.RemoveObject(item.root.gameObject); } catch (Exception e) {
