...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter {
    public abstract partial class SRIA<ViewModelType, TItemViewsHolder> : BaseView, ISRIA
    ...
    where TItemViewsHolder : BaseItemViewsHolder {
        IEnumerator SmoothScrollProgressCoroutine(
            ...
            // setting a value because of compiler, but it's initialized at least once in the loop below
            double initialVrtInsetFromParent = -1d,
                targetVrtInsetFromParent = -1d;
            bool needToCalculateInitialInset = true,
                needToCalculateTargetInset = true,
                notCanceledByCaller = true;
            ...
            do {
                ...
                value = initialVrtInsetFromParent * (1d - progress) + targetVrtInsetFromParent * progress;
        ...
        void ComputeVisibilityForCurrentPosition(
            bool forceFireScrollViewPositionChangedEvent,
            bool virtualizeContentPositionIfNeeded,
            bool alsoCorrectTransversalPositions,
            double overrideScrollingDelta) {
            ...
            ComputeVisibilityForCurrentPosition(
                forceFireScrollViewPositionChangedEvent,
                virtualizeContentPositionIfNeeded,
                alsoCorrectTransversalPositions);
        }
        ...
        void ComputeVisibilityForCurrentPosition(
            ...
            if (pev != null) {
                Utils.ForceSetPointerEventDistanceToZero(pev);
        ...
        void ComputeVisibilityTwinPass(ref PointerEventData pev, double delta, bool returnPointerEventDataIfNeeded) {
            ...
            if (returnPointerEventDataIfNeeded) {
                pev = Utils.GetOriginalPointerEventDataWithPointerDragGO(_Params.scrollRect.gameObject);
            }
        ...
        bool VirtualizeContentPositionIfNeeded(ref PointerEventData pev, double delta, bool returnPointerEventDataIfNeeded, bool alsoCorrectTransversalPositions) {
            ...
            if (!looped) {
                _InternalState.contentPanelSkippedInsetDueToVirtualization +=
                    contentInsetFromVPStart_Prev - _Params.content.GetInsetFromParentEdge(_Params.viewport, _InternalState.startEdge);
            }
        ...
        void ComputeVisibility(double abstractDelta) {
            ...
            int endItemIndexInView, neg1_posMinus1, neg1_pos0, neg0_pos1;

            neg1_posMinus1 = negativeScroll ? 1 : -1;
            ...
            if (thereWereVisibletems) {
                ...
                // startingLVHolderIndex will be: _VisibleItemsCount - 1, if negativeScroll 0, if upScroll
                ...
                // vItemHolder is: first in _VisibleItems, if negativeScroll last in _VisibleItems, else
                ...
                while (true) {
                    curRecCandidateSizePlusSpacing =
                        _ItemsDesc[curRecCandidateVH.itemIndexInView] + ctSpacing; // major bugfix: 18.12.2016 1:20: must use vItemHolder.ItemIndex INSTEAD of currentLVItemIndex
            ...
            if (Math.Abs(abstractDelta) > 10000d // huge jumps need optimization
                && (estimatedAVGVisibleItems = (int)Math.Round(Math.Min(_InternalState.viewportSize /
                ((_Params.DefaultItemSize + _InternalState.spacing)), _AVGVisibleItemsCount)))
                ...
                while (negInsetStart_posInsetEnd <= negCTVrtAmountBeforeVP_posCTVrtAmountAfterVP - itemSize) {
            ...
            do {
                ...
                do {
                    ...
                    if (negNLVCandidateIsBeforeVP_posNLVCandidateIsAfterVP) {
                        ...
                        if (negCTVrtInsetFromVPS_posCTVrtInsetFromVPE +
                            negCurrentVrtInsetFromCTSToUseForNLV_posCurrentVrtInsetFromCTEToUseForNLV > vpSize) {
                            breakBigLoop = true;
                            break;
                        }
                ...
                UIManager.SetUICanvasGroupEnable(nlvHolder.canvasGroup, true);
                nlvSize = _ItemsDesc[nlvIndexInView];
            ...
            for (int i = 0; i < _RecyclableItems.Count;) {
                ...
                //go.SetActive(false);
                UIManager.SetUICanvasGroupEnable(vh.canvasGroup, false);
                if (ShouldDestroyRecyclableItem(vh, GetNumExcessObjects() > 0)) {
                    //GameObject.Destroy(go);
                    PoolManager.RemoveObject(go);
                    _RecyclableItems.RemoveAt(i);
        ...
        void UpdateGalleryEffectIfNeeded() {
            ...
            float viewportPivot_MinimumIsMinus1_MaximumIs1 =
                _Params.galleryEffectViewportPivot * 2 - 1f;
        ...
        /// <inheritdoc/>
        class InternalState : InternalStateGeneric<BaseParams, TItemViewsHolder> {
            internal static InternalState CreateFromSourceParamsOrThrow(
                BaseParams sourceParams, ItemsDescriptor itemsDescriptor) {
                return new InternalState(sourceParams, itemsDescriptor);
            }

            public override void Updateparams(BaseParams _params) {
                base.Updateparams(_params);
            }


            protected InternalState(BaseParams sourceParams,
                ItemsDescriptor itemsDescriptor) : base(sourceParams, itemsDescriptor) { }
        }
    }
}
