...
namespace frame8.ScrollRectItemsAdapter.Util.GridView {
    ...
    public abstract class GridAdapter<ViewModelType, TCellVH> : SRIA<ViewModelType, CellGroupViewsHolder>
        where ViewModelType : BaseViewModel
        where TCellVH : BaseItemViewsHolder, new() {
        ...
        private GridParams _ParamsOfGrid;

        protected void GridAdapterInit(CustomScrollRect customScrollRect, GridLayoutGroup gridLayout,
            string cellPrefabPath, BaseParams.ContentGravity contentGravity = BaseParams.ContentGravity.START) {
            _ParamsOfGrid = new GridParams() {
                customScrollRect = customScrollRect,
                cellPrefabPath = cellPrefabPath,
                gridLayout = gridLayout,
                numCellsPerGroup = gridLayout.constraintCount,
                childAlignment = gridLayout.childAlignment
            };
            base.SRIAInit(customScrollRect, gridLayout.padding, gridLayout.spacing.y, contentGravity,
        ...
        public override void ChangeItemsCount(
            ...
            int groupsCount = _ParamsOfGrid.GetNumberOfRequiredGroups(_CellsCount);
            base.ChangeItemsCount(changeMode,
                groupsCount, indexIfAppendingOrRemoving,
        ...
        /// </summary>
        public sealed override AbstractViewsHolder GetViewsHolderOfClosestItemToViewportPoint(
            float viewportPoint01, float itemPoint01, out float distance) {
            var groupVH = base.GetViewsHolderOfClosestItemToViewportPoint(viewportPoint01, itemPoint01, out distance) as CellGroupViewsHolder;
        ...
        public sealed override void ScrollTo(int cellIndex,
            float normalizedOffsetFromViewportStart = 0,
            float normalizedPositionOfItemPivotToUse = 0) {
            ScrollToGroup(_ParamsOfGrid.GetGroupIndex(cellIndex),
                normalizedOffsetFromViewportStart, normalizedPositionOfItemPivotToUse);
        }
        ...
        public sealed override bool SmoothScrollTo(int cellIndex,
            float duration, float normalizedOffsetFromViewportStart = 0f,
            float normalizedPositionOfItemPivotToUse = 0f,
            Func<float, bool> onProgress = null, bool overrideAnyCurrentScrollingAnimation = false) {
            return SmoothScrollToGroup(
                _ParamsOfGrid.GetGroupIndex(cellIndex),
                duration, normalizedOffsetFromViewportStart,
                normalizedPositionOfItemPivotToUse,
                onProgress,
                overrideAnyCurrentScrollingAnimation);
        }
        ...
        public virtual void Refresh(bool contentPanelEndEdgeStationary, bool keepVelocity = false) {
            ChangeItemsCount(
                ItemCountChangeMode.RESET,
                _CellsCount, -1,
                contentPanelEndEdgeStationary,
                keepVelocity);
            this.SetVirtualAbstractNormalizedScrollPosition(
                contentPanelEndEdgeStationary ? 0f : 1f, false);
        }
        ...
        public virtual int GetNumVisibleCells() {
            if (_VisibleItemsCount == 0)
                return 0;
            return (_VisibleItemsCount - 1) * _ParamsOfGrid.numCellsPerGroup +
                _VisibleItems[_VisibleItemsCount - 1].NumActiveCells;
        }
        ...
        public virtual TCellVH GetCellViewsHolder(int cellViewsHolderIndex) {
            ...
            return _VisibleItems[_ParamsOfGrid.GetGroupIndex(cellViewsHolderIndex)]
                    .ContainingCellViewsHolders[cellViewsHolderIndex % _ParamsOfGrid.numCellsPerGroup] as TCellVH;
        }
        ...
        public virtual TCellVH GetCellViewsHolderIfVisible(int withCellItemIndex) {
            var groupVH = GetItemViewsHolderIfVisible(_ParamsOfGrid.GetGroupIndex(withCellItemIndex));
            if (groupVH == null)
                return null;

            int indexOfFirstCellInGroup = groupVH.ItemIndex * _ParamsOfGrid.numCellsPerGroup;

            if (withCellItemIndex < indexOfFirstCellInGroup + groupVH.NumActiveCells)
                return groupVH.ContainingCellViewsHolders[withCellItemIndex - indexOfFirstCellInGroup] as TCellVH;
        ...
        public virtual void ScrollToGroup(int groupIndex, float normalizedOffsetFromViewportStart = 0f, float normalizedPositionOfItemPivotToUse = 0f) { base.ScrollTo(groupIndex, normalizedOffsetFromViewportStart, normalizedPositionOfItemPivotToUse); }
        ...
        protected override CellGroupViewsHolder CreateViewsHolder(int itemIndex) {
            GameObject holder = GameObject.Instantiate(_ParamsOfGrid.GetGroupPrefab());
            holder.SetActive(true);
            Transform root = holder.transform as RectTransform;
            root.SetParent(_ParamsOfGrid.customScrollRect.content);
            root.localPosition = Vector3.zero;
            root.localScale = Vector3.one;

            var instance = holder.AddComponent<CellGroupViewsHolder>();
            if (instance == null) {
            ...
            instance.Init(
                holder,
                itemIndex,
                _ParamsOfGrid.cellPrefabPath,
                _ParamsOfGrid.numCellsPerGroup);
        ...
        protected sealed override void UpdateViewsHolder(CellGroupViewsHolder newOrRecycled) {
            ...
            if (newOrRecycled.ItemIndex + 1 == GetCellGroupsCount()) {
                ...
                if (newOrRecycled.ItemIndex > 0) {
                    totalCellsBeforeThisGroup = newOrRecycled.ItemIndex * _ParamsOfGrid.numCellsPerGroup;
                }
                activeCellsForThisGroup = _CellsCount - totalCellsBeforeThisGroup;
            } else {
                activeCellsForThisGroup = _ParamsOfGrid.numCellsPerGroup;
            }
            newOrRecycled.NumActiveCells = activeCellsForThisGroup;

            for (int i = 0; i < activeCellsForThisGroup; ++i)
                UpdateCellViewsHolder(newOrRecycled.ContainingCellViewsHolders[i] as TCellVH);
        }
        ...
        protected sealed override void OnBeforeRecycleOrDisableViewsHolder(
            CellGroupViewsHolder inRecycleBinOrVisible, int newItemIndex) {
            ...
            if (newItemIndex == -1)
                for (int i = 0; i < inRecycleBinOrVisible.NumActiveCells; ++i)
                    OnBeforeRecycleOrDisableCellViewsHolder(
                        inRecycleBinOrVisible.ContainingCellViewsHolders[i] as TCellVH, -1);
            else
                for (int i = 0; i < inRecycleBinOrVisible.NumActiveCells; ++i)
                    OnBeforeRecycleOrDisableCellViewsHolder(
                        inRecycleBinOrVisible.ContainingCellViewsHolders[i] as TCellVH, newItemIndex * _ParamsOfGrid.numCellsPerGroup + i);
        }
