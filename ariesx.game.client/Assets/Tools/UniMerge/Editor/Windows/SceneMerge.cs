...
namespace UniMerge.Editor.Windows {
	...
	public class SceneMerge : UniMergeWindow {
		...
		public static void PrefabMerge(string myPath, string theirPath) {
			...
			window.root.mine = (GameObject) AssetDatabase.LoadAssetAtPath(myPath, typeof(GameObject));
			window.root.theirs = (GameObject) AssetDatabase.LoadAssetAtPath(theirPath, typeof(GameObject));
		}
		...
		public IEnumerator MergeAsync(string myPath, string theirPath) {
			...
			if (AssetDatabase.LoadAssetAtPath(myPath, typeof(UnityObject))
				...
				if(EditorApplication.SaveCurrentSceneIfUserWantsTo()) {
					...
					objectMerge.root.mine = myContainer;
					objectMerge.root.theirs = theirContainer;
					objectMerge.update = objectMerge.root.Refresh();
		...
		void MergeScenes(string myPath, string theirPath) {
			...
			myContainer = new GameObject { name = myContainerName };
			...
			foreach (var obj in allObjects)
				if (obj.transform.parent == null && obj.name == myContainerName
					...
					&& PrefabUtility.GetPrefabType(obj) != PrefabType.ModelPrefab && obj.hideFlags == 0) {
					//Want a better way to filter out "internal" objects
					myContainer = obj;
					break;
				}
			...
			theirContainer = new GameObject { name = theirContainerName };
		...
		public override IEnumerator Refresh() {
			var enumerator = PropertyHelper.UpdatePropertyList(properties, new SerializedObject(mySceneData),
				new SerializedObject(theirSceneData), window: this);
			while (enumerator.MoveNext())
				yield return null;

			foreach (var helper in properties) {
				var tmp = new List<PropertyHelper>(helper.children);
				foreach (var h in tmp)
					if (h.property.propertyPath.Contains("sunPath"))
						helper.children.Remove(h);
			}
		}

		protected override int GetDrawCount() { return properties.Sum(propertyHelper => propertyHelper.GetDrawCount()); }
	}
}
